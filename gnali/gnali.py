...
GNOMAD_DBS = ["https://storage.googleapis.com/gnomad-public/release/2.1.1/vcf/exomes/gnomad.exomes.r2.1.1.sites.vcf.bgz"]
# To run gNALI on both gnomAD exome and genome databases, add GNOMAD_GENOMES to GENOMAD_DBS above.
# (~15min runtime)
...
def get_plof_variants(start_dir, temp_dir, *databases):
	...
	for database in databases:
		database_short = database[-20:-6]
		print(os.system("xargs -a test_locations.txt -I {} tabix -fh " + database + " {} > exomes_Results_" + database_short + ".vcf"))
		print(os.system("bgzip exomes_Results_" + database_short + ".vcf"))
		os.system("zgrep -e ';controls_nhomalt=[1-9]' exomes_Results_" + database_short + ".vcf.gz > exomes_R_Hom_" + database_short + ".txt")
		os.system("grep -e HC exomes_R_Hom_" + database_short + ".txt > exomes_R_Hom_HC_" + database_short + ".txt")


def write_results(out_file, temp_dir, start_dir, results_dir):
	# Write results generated by get_plof_variants()
	os.chdir(temp_dir.name)
	results = []
	for database in GNOMAD_DBS:
		results.append(pd.read_table("exomes_R_Hom_HC_" + database[-20:-6] + ".txt"))
...
def main():
	...
	write_results(args.output_file, temp_dir, start_dir, results_dir)
